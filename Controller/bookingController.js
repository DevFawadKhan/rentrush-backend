...
export const updateBooking = async (req, res) => {
  ...
  const userId = req.user; // Assuming req.user contains the authenticated userId

  try {
    // Find the booking by ID and verify it belongs to the current user
    const booking = await Booking.findOne({ _id: bookingId, userId });
    if (!booking) {
      return res.status(404).json({ message: 'Booking not found or unauthorized access.' });
    }

    // Check if the car is still rented out
    const car = await Car.findById(booking.carId);
    if (!car || car.availability !== 'Rented Out') {
      return res.status(400).json({ message: 'Car is not currently rented out.' });
    }
    ...
    // Ensure the rentalEndDate is after rentalStartDate
...
export const cancelBooking = async (req, res) => {
  ...
  try {
    ...
    // Check if the cancellation request is within the allowable time frame
    const hoursUntilStart = moment(booking.rentalStartDate).diff(moment(), 'hours');
    if (hoursUntilStart < 24) {
      return res.status(400).json({
        message: 'Cancellation not allowed within 24 hours of rental start date.'
      });
    }
